...
import { canCompleteTask, formatTaskTime, getTimeUntilCompletion, isOfferOpenForApplications, getTimeUntilApplicationsClose } from '@/lib/dateUtils';
import {
  ArrowLeft,
  MapPin,
  Clock,
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                {offer.status === 'published' && (
                  <div className="flex items-center">
                    <div className={`w-5 h-5 rounded-full mr-3 ${isOfferOpenForApplications(offer.dateTime) ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    <span className="text-gray-700">
                      Postulaciones: {isOfferOpenForApplications(offer.dateTime) ? 'Abiertas' : 'Cerradas'}
                      {isOfferOpenForApplications(offer.dateTime) && (
                        <span className="text-sm text-gray-500 ml-2">
                          (Cierran en {getTimeUntilApplicationsClose(offer.dateTime)})
                        </span>
                      )}
                    </span>
                  </div>
